    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if opt.get('val') is not None and (current_iter %
                ...
                # ================= 新增：自动保存最佳 PSNR 权重 =================
                if hasattr(model, 'metric_results') and 'psnr' in model.metric_results:
                    current_psnr = model.metric_results['psnr']
                    
                    if current_psnr > best_psnr:
                        best_psnr = current_psnr
                        best_iter = current_iter
                        logger.info(f'\n[Best Model Update] 新的最佳 PSNR: {best_psnr:.4f} (Iter: {best_iter}). 正在保存最佳权重...\n')
                        
                        # BasicSR 自带的保存网络功能
                        if hasattr(model, 'save_network'):
                            model.save_network(model.net_g, 'net_g', 'best')
                        else:
                            # 备用原生保存方法
                            save_path = osp.join(opt['path']['models'], 'net_g_best.pth')
                            torch.save(model.net_g.state_dict(), save_path)
                    else:
                        logger.info(f'\n[PSNR 未突破] 当前 PSNR: {current_psnr:.4f} | 历史最佳 PSNR: {best_psnr:.4f} (Iter: {best_iter})\n')
                # ================================================================
